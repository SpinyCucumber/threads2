...
const pieces = new PieceSet([
    ...
    // Straights
    new Piece(2, 0b100100, 4),
    new Piece(7, 0b010010, 0.125),
    new Piece(11, 0b001001, 0.125),
...
const r = 7;
const space = enumerateSpiral(new CubePosition({ q: 0, r: 0, s: 0 }), r);
...
const start = new CubePosition({ q: -r, r: 0, s: r });
const end = new CubePosition({ q: r, r: 0, s: -r });
...
const collapser = new Collapser({
    ...
    constraints: [
        ...
        pieces.createFilterConstraint(Seq(pieces).map(piece => piece.id).toSet().subtract([15]), (position) => position.equals(start)),
        pieces.createFilterConstraint(Seq(pieces).map(piece => piece.id).toSet().subtract([18]), (position) => position.equals(end)),
        pieces.createFilterConstraint([15], (position) => !position.equals(start)),
        // pieces.createFilterConstraint([21], (position) => (position.q === -position.s)),
    ],
